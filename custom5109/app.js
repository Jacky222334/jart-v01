...
function frame(now) {
  ...
  const a = audio.tick();
  if (running) {
    drawFrame(ctx, window.innerWidth, window.innerHeight, t, a);
  }
  meters.bass.style.transform = `scaleX(${a.bass})`;
  meters.mid.style.transform = `scaleX(${a.mid})`;
  meters.treble.style.transform = `scaleX(${a.treble})`;
...
canvas.addEventListener('click', () => {
  ...
  statusEl.textContent = running
    ? (audio.isActive() ? `live · ${audio.getLabel()}` : 'idle · klick für Spotify')
    : 'pause';
});
